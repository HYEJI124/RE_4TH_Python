...
int switch2= 10; // 택트 스위치 2번과 연결된 디지털 핀 번호
int ledRed = 4; // 택트 스위치 1번과 연결된 빨간색 LED 핀 번호
int ledBlue = 2; // 택트 스위치 2번과 연결된 파란색 LED 핀 번호

void setup() {
   Serial.begin(9600); // 9600 속도로 시리얼 통신을 시작
   pinMode(switch1, INPUT_PULLUP); // switch 1번 핀을 INPUT_PULLUP으로 시작
   pinMode(switch2, INPUT_PULLUP); // switch 2번 핀을 INPUT_PULLUP으로 시작
   pinMode(ledBlue , OUTPUT); // ledBlue번 디지털 핀으로 led를 켜기 위해 전력을 출력
   pinMode(ledRed , OUTPUT); // ledRed번 디지털 핀으로 led를 켜기 위해 전력을 출력
}
