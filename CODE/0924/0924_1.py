...
# 메모리 비효율적 - 전체 파일을 메모리에 올림
print('1. read() - 파일 전체를 하나의 문자열로')
with open('data.txt', 'r', encoding='utf-8') as f:
    content = f.read() # 전체 내용
    print(content)     # 10GB 파일이면 메모리 10GB 사용!
...
with open('data.txt', 'r', encoding='utf-8') as f:
    ...
    print(f'첫 번째 줄 읽은 후 위치: {f.tell()}')
    line2 = f.readline()
    print(line2.strip())
    print(f'두 번째 줄 읽은 후 위치: {f.tell()}')
    f.seek(18) # 처음으로
    line3 = f.readline()
    print(line3.strip())
    print(f'세 번째 줄 읽은 후 위치: {f.tell()}')
    line4 = f.readline()
    print(line4.strip())
    print(f'네 번째 줄 읽은 후 위치: {f.tell()}')
print()
...
with open('test.jpeg', 'rb') as f1:
    img = f1.read()


with open('./test_copy.jpeg', 'wb') as f2:
    f2.write(img)